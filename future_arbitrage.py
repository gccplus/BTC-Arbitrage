...
class TermArbitrage:
    ...
    # TODO
    # 增加动态调整
    open_positive_thd = 45
    close_positive_thd = -5

    open_negative_thd = -100
    close_negetive_thd = -50
    ...
    def asset_balance(self):
        ...
        total_btc = self.future_balance + self.spot_free_btc
        future_prop = 1 / (1 + 10 * self.margin_coefficient)
        trans_amount = abs(self.future_balance - total_btc * future_prop)
        if trans_amount < 0.0001:
            self.logger.info('平衡状态,无需转移')
            return
        if self.future_balance > total_btc * future_prop:
            self.logger.info('合约转币币: %s' % trans_amount)
            trans_amount = self.future_balance - total_btc * future_prop
            try:
        ...
        try:
            ...
            if info['result']:
                ...
                if len(holding) > 0:
                    ...
                    self.buy_profit_lossratio = holding[0]['buy_profit_lossratio']
                    self.sell_profit_lossratio = holding[0]['sell_profit_lossratio']
                    self.buy_price_avg = holding[0]['buy_price_avg']
                    self.sell_price_avg = holding[0]['sell_price_avg']
                    self.logger.info('多仓: %s\t空仓: %s' % (self.bull_amount, self.bear_amount))
                else:
    ...
    def update_account_info(self):
        ...
        try:
            ...
            print spot_info
            if spot_info['result']:
                self.spot_free_btc = float(spot_info['info']['funds']['free']['btc'])
                self.spot_free_usdt = float(spot_info['info']['funds']['free']['usdt'])
                self.spot_freezed_btc = float(spot_info['info']['funds']['freezed']['btc'])
                self.spot_freezed_usdt = float(spot_info['info']['funds']['freezed']['usdt'])
    ...
    def go(self):
        while True:
            ...
            if self.debug_type == 2 or future_bids[0][0] - float(spot_asks[0][0]) > self.open_positive_thd:
                ...
                self.logger.info('期货价格: %s,现货价格 %s' % (future_bids[0][0], spot_asks[0][0]))

                avialable_bear_amount = int(
                    self.future_balance_fix * future_bids[0][0] / 10)
                ...
                if spot_usdt_amount > self.spot_free_usdt:
                    self.logger.info('现货USDT数量%s, 不足%s,本次交易取消' % (self.spot_free_usdt, spot_usdt_amount))
                    continue
                # 限价购买期货合约
                ...
                try:
                    ...
                    self.logger.error('获取订单信息异常: %s' % e)
                    continue
                print order_info
                        ...
                        while times < 10:
                            try:
                                order_info = self.future_client.order_info(self.symbol, self.contract_type,
                                                                           0, orderid, 1, 5)
                                order_info = order_info['orders'][0]
                                self.logger.info('当前订单状态: %s' % order_info['status'])
                                # 若撤销失败，状态必定为完全成交
                ...
                # btc精度是8
                future_deal_btc_amount = float('%.8f' % (future_deal_contract_amount * 10 / future_deal_price))

                self.bear_amount += future_deal_contract_amount

                self.future_balance_fix -= future_deal_btc_amount
                self.future_balance_fix -= future_deal_fee

                self.logger.info('future_balance_fix剩余: %s' % self.future_balance_fix)
                ...
                try:
                    spot_order = self.spot_client.place_order('', spot_usdt_amount, 'buy_market', self.symbol)
                except Exception as e:
            ...
            if self.debug_type == 1 or future_asks[0][0] - float(spot_bids[0][0]) < self.open_negative_thd:
                ...
                self.logger.info('期货价格: %s,现货价格 %s' % (future_asks[0][0], spot_bids[0][0]))
                avialable_bull_amount = int(
                    self.future_balance_fix * future_asks[0][0] / 10)
                ...
                spot_sum = np.sum(a[:3], axis=0)
                spot_std = np.std(a[:3], axis=0)
                self.logger.info('BIDS:\tsum:%10.4f\tstd:%10.4f' % (spot_sum[1], spot_std[0]))
                if spot_sum[1] < 0.5 or spot_std[0] > 5:
                ...
                spot_limited_price = float(spot_bids[0][0]) - 10
                spot_btc_amount = float('%.4f' % (100 * future_contract_amount / spot_limited_price))

                if spot_btc_amount > self.spot_free_btc:
                    self.logger.info('现货BTC数量%s, 不足%s,本次交易取消' % (self.spot_free_btc, spot_btc_amount))
                    continue
